...
void motorControl(bool ifLeftMotor, char command) {
	...
	int polarity    = ifLeftMotor ? pol_left : pol_right; 

    switch (command) {
        case FORWARD:
            gpio_set_value(enable, SPEED);
            gpio_set_value(motorPos, !polarity); // 1
            gpio_set_value(motorNeg, polarity);  // 0
            break;
        case BACK:
            gpio_set_value(enable, SPEED);
            gpio_set_value(motorPos, polarity);  // 0
            gpio_set_value(motorNeg, !polarity); // 1
            break;    
        case STOP:
...
// everything under here was added
long memory_ioctl (struct file *filp, unsigned int cmd, unsigned long arg) {
